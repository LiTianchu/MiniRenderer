...
void draw_mesh_shaded(Model model, TGAImage &image, Vec3f light_dir);
...
Vec3f get_barycentric(Vec2i *pts, Vec2i P);
...
int main(int argc, char **argv)
{
    ...
    // sort_triangle_vertices(t0);
    // sort_triangle_vertices(t1);
    // sort_triangle_vertices(t2);
    // std::cout << "t0 = " << t0[0] << " " << t0[1] << " " << t0[2] << std::endl;
    // std::cout << "t1 = " << t1[0] << " " << t1[1] << " " << t1[2] << std::endl;
    // std::cout << "t2 = " << t2[0] << " " << t2[1] << " " << t2[2] << std::endl;

    // triangle_linesweeping(t0[0], t0[1], t0[2], image, red);
    // triangle_linesweeping(t1[0], t1[1], t1[2], image, white);
    // triangle_linesweeping(t2[0], t2[1], t2[2], image, green);
    ...
    if(argc >= 2){
        ...
        if(std::string(argv[1]) == "wireframe"){
            ...
            draw_mesh_shaded(model_loaded, image, Vec3f(0,0,-1));
        }else{
...
void draw_mesh_shaded(Model model, TGAImage &image, Vec3f light_dir){
    ...
    for (int i = 0; i < model.nfaces(); i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            Vec3 vertex_world = model.vert(face[j]);
            ...
            t_screen[j] = Vec2i(x_screen, y_screen);
        ...
        if (intensity > 0)
        {
            triangle_barycentric(t_screen,image,TGAColor(255 * intensity,255 * intensity,255 * intensity,255));
        }
    }
}
